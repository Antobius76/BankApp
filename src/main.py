...
from src.search_transactions import list_dict_with_transaction
...
def main() -> None:
    ...
    while True:
        ...
        if user_file_choice == "1":
            ...
            list_transactions = transactions_from_csv(os.path.join("../src/transactions.csv"))
            break
        elif user_file_choice == "3":
            print("Выбран XLSX-файл.")
            list_transactions = transactions_from_excel(os.path.join("../data/transactions_excel.xlsx"))
            break
        else:
    ...
    for filter_type, filter_value in list_transactions.items():
        if filter_type == "status":
            ...
            transactions = list_dict_with_transaction(transactions, filter_value)
